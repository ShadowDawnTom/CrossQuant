...
7. 一次结算组等待两条腿都至少完成一次模拟结算后退出；`rolling_v2` 滚动组普通持有价值不为正需连续 30 次有效扫描、资金费方向反转需连续 15 次有效扫描，恢复后对应计数清零；普通收益退出仍必须先观察两条腿各一次结算；
...
12. 滚动组平仓后至少冷却 8 小时，并等待两条腿都跨过各自下一次结算点；旧版数据保留为 `rolling_v1`，但不与 `rolling_v2` 的当前统计混算。
...
- 研究模拟 `rolling_v2` 中，普通继续持有价值不足需要连续 30 次有效扫描确认，资金费现金流方向反转需要连续 15 次有效扫描确认；行情降级不增加计数，优势恢复则清零；
