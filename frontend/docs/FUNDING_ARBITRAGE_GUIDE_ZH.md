...
7. 一次结算组等待两条腿都至少完成一次模拟结算后退出；`rolling_v4` 滚动组第一次双腿结算前不累计普通退出计数，结算后普通持有价值不为正需连续 60 次有效扫描、资金费方向反转需连续 30 次有效扫描，恢复后对应计数清零；
...
12. 滚动组平仓后至少冷却 12 小时，并等待两条腿都跨过各自下一次结算点；`rolling_v1`～`rolling_v3` 只保留为审计历史，不再生成新仓位，也不与 `rolling_v4` 当前统计混算。
...
- 研究模拟 `rolling_v4` 中，第一次双腿结算前不累计普通退出计数；结算后普通继续持有价值不足需要连续 60 次有效扫描确认，资金费现金流方向反转需要连续 30 次有效扫描确认；行情降级不增加计数，优势恢复则清零；
