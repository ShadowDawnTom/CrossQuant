...
1. 严格实盘扫描仍只覆盖 `GCT_FUNDING_SCAN_ASSETS`；DOGE、TRUMP 等扩展资产只进入研究扫描；
2. 每轮把所有组合的同步、规则、深度、滑点和成本检查结果保存到 `funding_scan_observations`；
3. 同时标记“达到实盘标准”“只达到研究标准”和“完全拒绝”，保存主要拒绝原因；
4. 研究资格仍要求 `LIVE_SYNCHRONIZED`、合约可交易、共同数量合法、开平四向深度足够且滑点不超限；
5. 即使扣费后净收益为负，也从本轮研究资格组合中选择净收益最高的一组；
6. 全局唯一索引限制最多一组 `OPEN` 研究仓位，目标单腿 5 USDT，但实际金额会按两所共同最小数量向上取整；
7. 持仓至少经历一次模拟结算事件后，才按当时可执行盘口模拟平仓；
8. 开仓、平仓、四笔手续费、价差损益、资金费、基差、滑点和最终 PnL 全部单独保存。
...
- 普通的继续持有价值不足需要连续三轮确认；资金费现金流方向已经反转时立即退出；
...
- 最新一轮每个组合的原始资金费、保守资金费、四笔手续费、盘口往返损益、压力缓冲和净收益；
- 实盘拒绝原因、盈亏平衡持有小时数、合法数量与实际单腿名义金额；
- 当前和历史 `RESEARCH` 持仓、模拟结算与评估时间线。
...
Kraken、Hyperliquid、Deribit 可以出现在行情矩阵里，但不在当前四所自动执行状态机范围内。
