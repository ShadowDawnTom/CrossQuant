...
7. 一次结算组只承认开仓之后、两条腿都真实跨过的结算时点；`rolling_v7` 滚动组至少观察 24 小时，期间普通持有价值不足只记录不退出，资金费方向持续反转仍可提前退出；24 小时后普通退出需连续 180 次有效扫描、方向反转需连续 60 次有效扫描，恢复后对应计数清零；
...
12. 滚动组平仓后至少冷却 12 小时，并等待两条腿都跨过各自下一次结算点；`rolling_v1`～`rolling_v6` 只保留为审计历史，其中 `rolling_v5` 存在开仓前结算归属缺陷，`rolling_v6` 使用旧发现池，两者都不与 `rolling_v7` 当前统计混算。

`rolling_v7` 的发现池不再按静态字母列表铺满 80 个长尾币，而是使用 38 个主流币、行业龙头和 RWA 龙头白名单。`SNDKX` 按 Gate 实际代码使用 `SNDK`，宇树科技使用 `UNITREE`，ZKsync 使用 `ZK`，微软使用 `MSFT`；`MSTRX`、`COINX`、`NVDAX`、`GOOGLX` 保留 X 合约身份，避免在未验证经济含义前与无 X 合约误合并。Hyperliquid 的 `SKHX` 仅在路由层映射为统一标的 `SKHYNIX`。逐所订阅还会按实时交易规则做白名单过滤，不存在的合约不会再发送 WebSocket 订阅或 REST 盘口初始化请求。
...
- 研究模拟 `rolling_v7` 中，任何不晚于开仓时刻的结算事件都会被滚动到下一合法周期或取消；系统直接查询并评估全部 `OPEN` 仓位，不再从最近十条历史记录中筛选；
