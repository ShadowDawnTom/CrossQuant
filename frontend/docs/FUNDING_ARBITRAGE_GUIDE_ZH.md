...
1. 严格实盘扫描仍只覆盖 `GCT_FUNDING_SCAN_ASSETS`；扩展发现池使用 38 个龙头常驻发现池和 40～60 个高费率卫星池，只读取资金费、Ticker、交易规则和持仓量；
...
6. 同一时点建立“一次结算验证组”“滚动持仓组”“固定72小时组”和“固定7天组”，每组最多三个不同 `OPEN` 组合；目标单腿 5 USD，实际金额按共同最小数量向上取整，但任一腿超过 10 USD 就拒绝；
7. 一次结算组只承认开仓之后、两条腿都真实跨过的结算时点；`rolling_v8` 滚动组至少观察 24 小时，固定组不因短期费率翻转退出，用于隔离持仓时长对手续费回收的影响；
8. USD、USDC、USDT 价格先按实时中间价换算成 USD；汇率过期或脱锚时拒绝候选，跨报价组合还会额外扣稳定币风险缓冲；
9. 开仓、平仓、四笔手续费、价差损益、资金费、基差、滑点、优势持续时间、方向翻转次数和最终 PnL 全部单独保存；研究结算必须等历史接口返回最终费率，标记为 `REALIZED_HISTORY`，缺值时保持 `PENDING`；
10. 后端还并行保存 Taker/Taker、按成交概率折算的 Maker/Taker 反事实，以及 Gate/Binance/OKX 同所现货/永续现金套利；反事实不会进入实盘下单器；
11. 命中率来自 `funding_rate_history` 中两条腿都有真实结算数据的完整 UTC 日。系统分别构造 24 小时、72 小时和 7 天连续窗口，用“窗口内每天方向都保持为正”的生存概率折算多周期收益；样本不足 7 个窗口时使用人工保守先验，样本足够后只允许真实生存率把预期调低。
12. 滚动组平仓后至少冷却 12 小时，并等待两条腿都跨过各自下一次结算点；`rolling_v1`～`rolling_v7` 只保留为审计历史，不与 `rolling_v8` 当前统计混算。

`rolling_v8` 的发现池不按字母截断：38 个主流币、行业龙头和 RWA 龙头常驻轻量发现，55 个卫星币按毛费率差、持续确认和持仓量评分竞争热池。`SNDKX` 按 Gate 实际代码使用 `SNDK`，宇树科技使用 `UNITREE`，ZKsync 使用 `ZK`，微软使用 `MSFT`；`MSTRX`、`COINX`、`NVDAX`、`GOOGLX` 保留 X 合约身份。Hyperliquid HIP-3 合约通过实时元数据路由到 `dex:coin`（例如 `xyz:UNITREE`），统一 CrossEx symbol 仍保持无前缀。热池采用“两阶段切换”：新池逐币至少取得两所实时盘口后才撤旧订阅，超时继续使用旧池。
...
- 入场时同时计算 24 小时、72 小时和 7 天的真实结算事件，将当前费率路径按历史生存概率折算，再扣除四笔手续费、当前往返盘口成本、滑点压力和基差缓冲；只有至少一个周期净值和净年化都达标才进入严格候选；
- 开仓后每分钟只判断未来一至两个真实结算事件的边际价值。已经支付的开仓手续费是沉没成本，不会每分钟重复扣除；
- `继续持有价值 = 未来结算保守资金费 - 新增滑点压力 - 新增基差风险缓冲`；
- 研究模拟 `rolling_v8` 中，任何不晚于开仓时刻的结算事件都会被滚动到下一合法周期或取消；结算只认最终历史费率，系统直接查询并评估全部 `OPEN` 仓位；
