...
flowchart TD
    ...
    M --> Q[费率、保证金、ADL、持仓时间监控]
    Q --> R[两腿 reduce-only 平仓]
...
2. 按每个交易所真实结算间隔，计算未来 24 小时有几次现金流；
3. 找出资金费总额较低的一侧做多、较高的一侧做空；
4. 读取每家交易所的 taker 手续费；
5. 使用多档订单簿计算目标数量的开仓和假设立即平仓价格；
6. 扣除四笔手续费、进出场基差和盘口成本；
7. 只接受 `LIVE_SYNCHRONIZED` 行情；
8. 同一组合连续三轮通过，才成为 `CONFIRMED` 候选。
...
当前前端不会展开显示候选的净年化、确认次数和最后更新时间。点击前应由运维/API 或后续候选详情 UI 再核对，不能只依据下方矩阵的 APR。
...
- Telegram 代码已部署，但真实 Token/Chat ID 尚未配置；
...
2. 把候选详情、确认次数、净年化、拒绝原因和更新时间展示到前端；
3. 完成 24–48 小时观察报告；
4. 等待一个扣除成本后仍为正的 `CONFIRMED` 候选；
5. 根据该币种的共同数量步长确定 Canary 金额；
6. 人工在线执行一次 Canary；
7. 完整对账后再决定是否提高额度。
