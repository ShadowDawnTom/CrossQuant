...
| `GCT_FUNDING_RESEARCH_ASSETS` | 30 个高流动性候选币 | 研究资产；启用时必须同时配置对应执行订单簿，最多 50 个 |
| `GCT_FUNDING_RESEARCH_TARGET_NOTIONAL_USD` | `5` | 单腿目标金额；仍按共同最小数量向上取整 |
| `GCT_FUNDING_RESEARCH_MAX_OPEN_POSITIONS` | `1` | 每个实验组固定最多 1 个；一次结算组和滚动组最多合计 2 个 |
...
| `GCT_FUNDING_RESEARCH_MODEL_VERSION` | `rolling_v2` | 当前研究账本版本；切换版本会隔离统计并归档旧版未平模拟仓位 |
| `GCT_FUNDING_RESEARCH_HOLD_EXIT_CONFIRMATIONS` | `30` | 滚动组继续持有价值不为正的连续有效扫描确认数，默认约 30 分钟 |
| `GCT_FUNDING_RESEARCH_REVERSAL_EXIT_CONFIRMATIONS` | `15` | 滚动组资金费方向反转的连续有效扫描确认数，默认约 15 分钟 |
| `GCT_FUNDING_RESEARCH_REENTRY_COOLDOWN_MS` | `28800000` | 研究组平仓后的最短重开冷却，实际还会等待两条腿都跨过下一结算点 |
...
GCT_EXECUTION_MARKET_SYMBOLS=BTC,ETH,SOL,XRP,DOGE,ADA,SUI,AVAX,LINK,LTC,BCH,DOT,UNI,AAVE,NEAR,APT,ARB,OP,INJ,ATOM,FIL,ETC,XLM,HBAR,TRX,SEI,JUP,TAO,WIF,PEPE
GCT_FUNDING_SCAN_ASSETS=BTC,ETH,SOL
GCT_FUNDING_RESEARCH_ENABLED=1
GCT_FUNDING_RESEARCH_ASSETS=BTC,ETH,SOL,XRP,DOGE,ADA,SUI,AVAX,LINK,LTC,BCH,DOT,UNI,AAVE,NEAR,APT,ARB,OP,INJ,ATOM,FIL,ETC,XLM,HBAR,TRX,SEI,JUP,TAO,WIF,PEPE
GCT_FUNDING_RESEARCH_TARGET_NOTIONAL_USD=5
GCT_FUNDING_RESEARCH_MAX_OPEN_POSITIONS=1
GCT_FUNDING_RESEARCH_MODEL_VERSION=rolling_v2
GCT_FUNDING_RESEARCH_HOLD_EXIT_CONFIRMATIONS=30
GCT_FUNDING_RESEARCH_REVERSAL_EXIT_CONFIRMATIONS=15
GCT_FUNDING_RESEARCH_REENTRY_COOLDOWN_MS=28800000
